public  partial class Raca
{
    ...
    public Raca(string _Nome, string _Informacoes=null)
    {
        ...
        {
            Console.WriteLine("Nome ", "Não foi possivel criar a Raca devido a dados invalidos no Nome");
        }
        else if ( _Informacoes!=null )
        {   
            if(!Validacoes.VerificarCampo(_Informacoes) ){
                Console.WriteLine("Informações", "Não foi possivel criar a Raca devido a dados invalidos nas Informações");
            }
                else
        {
            Nome = _Nome;
